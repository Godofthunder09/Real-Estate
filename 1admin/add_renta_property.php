...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_property'])) {
    ...
    // Target directory for rent property images
    $target_dir = $_SERVER['DOCUMENT_ROOT'] . "/jaga/landrecordsys/rent_upload/";
    ...
    if (!empty($_FILES['images']['name'][0])) {
        ...
        if ($file_count > 3) {
            ...
            if (!empty($image_paths)) {
                ...
                $insert_query = "INSERT INTO addrentproperty 
                    (user_id, property_id, owner_name, phone, email, property_type, location, land_area, images, rent_price, security_deposit, furnishing_status, availability_date, rental_duration)
                    VALUES 
                    ($user_id, '$property_id', '$owner_name', '$phone', '$email', '$property_type', '$location', '$land_area', '$image_string', '$rent_price', '$security_deposit', '$furnishing_status', '$availability_date', '$rental_duration')";

                if (mysqli_query($con, $insert_query)) {
                    echo "<script>alert('Rent property added successfully!'); window.location.href = 'admin_dashboard.php';</script>";
                    exit();
                } else {
...
<head>
    ...
    <style>
        body {margin:0; font-family:'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background:#121212; color:#fff; display:flex; height:100vh; overflow:hidden;}
        .sidebar {width:260px; background:linear-gradient(to bottom right,#14213d,#1a2a6c); padding:30px 20px; box-shadow:2px 0 5px rgba(0,0,0,0.3); flex-shrink:0;}
        .sidebar h2 {color:#9be1ff; margin-bottom:10px; font-size:20px; line-height:1.5; text-align:center;}
        .sidebar p {color:#cfd8dc; font-size:14px; margin-bottom:30px; text-align:center;}
        .sidebar a, .sidebar .submenu-toggle {display:flex; justify-content:space-between; align-items:center; padding:12px 20px; margin-bottom:15px; text-decoration:none; color:white; border-radius:8px; background-color:#2c2c54; transition:background 0.3s ease, transform 0.2s ease; font-weight:bold; cursor:pointer;}
        .sidebar a:hover, .sidebar .submenu-toggle:hover {background-color:#1565c0; transform:scale(1.05);}
        ...
        .triangle {width:0; height:0; border-left:6px solid transparent; border-right:6px solid transparent; border-top:8px solid white; transition:transform 0.3s ease;}
        .triangle.open {transform:rotate(180deg);}
        .main-content {flex-grow:1; background:#1e1e2f; padding:40px; overflow-y:auto;}
        .container {background:#2a2a40; padding:30px; border-radius:15px; box-shadow:0 0 20px rgba(0,150,255,0.2); max-width:600px; margin:auto;}
        input, select {width:100%; padding:10px; margin:10px 0; border:1px solid #444; border-radius:5px; background-color:#1e1e3f; color:#fff; font-size:14px; outline:none; transition:border 0.3s, box-shadow 0.3s;}
        input::placeholder {color:#aaa;}
        input[readonly] {background-color:#2e2e4d; color:#ccc;}
        input:focus, select:focus {border-color:#2196f3; box-shadow:0 0 5px rgba(33,150,243,0.5);}
        button {background:#28a745; color:white; padding:12px; width:100%; border:none; cursor:pointer; font-size:16px; border-radius:5px; margin-top:15px; transition:background 0.3s ease;}
...
<div class="main-content">
    <div class="container">
        <h3 style="text-align:center; color: #76c7f3;">Add New Rent Property</h3>
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <input type="text" name="security_deposit" value="0.00">
            ...
            <input type="text" name="rental_duration" required placeholder="e.g., 6 months, 1 year">
