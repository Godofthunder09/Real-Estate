...
$query = "SELECT * FROM addproperty WHERE 1";
...
if (!empty($filter_min_price)) {
    $query .= " AND price_range >= '" . mysqli_real_escape_string($con, $filter_min_price) . "'";
}

if (!empty($filter_max_price)) {
    $query .= " AND price_range <= '" . mysqli_real_escape_string($con, $filter_max_price) . "'";
}
...
<head>
    ...
    <style>
        ...
        /* Sidebar theme matches admin dashboard */
        .sidebar {
            width: 260px;
...
<div class="sidebar">
    ...
    <p>Browse your property listings.</p>
...
<div class="main-content">
    ...
    <form class="filter-form" method="GET">
        ...
        <select name="type">
            ...
            <option value="Agricultural" <?php if ($filter_type === 'Agricultural') echo 'selected'; ?>>Agricultural</option>
                ...
                <div class="property-card">
                    ...
                    <?php 
                        ...
                        $imagePaths = array_map(function($img) {
                            $img = trim($img);
                            return str_starts_with($img, "uploads/") ? $img : "uploads/" . $img;
                        }, $images);
                    ?>
...
<script>
    ...
    // Wishlist AJAX with popup
    function addToWishlist(element) {
        const propertyId = element.getAttribute("data-property-id");
        const xhr = new XMLHttpRequest();
        xhr.open("GET", "add_to_wishlist.php?property_id=" + propertyId + "&type=sale", true);
        xhr.onload = function () {
            if (xhr.status === 200) {
                const response = JSON.parse(xhr.responseText);
                showPopup(response.message);
            } else {
                showPopup('Failed to add property to wishlist.');
            }
        };
        xhr.send();
    }
